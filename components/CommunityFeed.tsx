...
export const CommunityFeed: React.FC<{ userProfile?: any }> = ({ userProfile }) => {
    ...
    const filteredObservations = observations.filter(obs => {
        // Search filter: check if search query matches plant name or caption
        if (searchQuery.trim()) {
            const query = searchQuery.toLowerCase().trim();
            const matchesPlantName = obs.specimen_taxonomy?.toLowerCase().includes(query);
            const matchesCaption = obs.caption?.toLowerCase().includes(query);
            if (!matchesPlantName && !matchesCaption) {
                return false;
            }
        }
